            ...
            <div class="card-body">
                <form action="/admin/siswa" method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nisn" class="form-control bg-light border-0" value="{{ old('nisn') }}" placeholder="Contoh: 0012345678" maxlength="10" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Nama Lengkap</label>
                        <input type="text" name="nama" class="form-control bg-light border-0" value="{{ old('nama') }}" placeholder="Nama lengkap siswa" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">Kelas</label>
                        <input type="text" name="kelas" class="form-control bg-light border-0" value="{{ old('kelas') }}" placeholder="Contoh: XII RPL 1" required>
                    </div>
                    <div class="mb-3">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-end pe-4">
                                    <button class="btn btn-sm btn-outline-primary shadow-sm rounded-pill px-3 me-1" data-bs-toggle="modal" data-bs-target="#editSiswa{{ $s->nisn }}">
                                        <i class="bi bi-pencil-square me-1"></i> Edit
                                    </button>
                                    <button type="button" class="btn btn-sm btn-outline-danger shadow-sm rounded-pill px-3" onclick="confirmDelete('/admin/siswa/{{ $s->nisn }}/delete', 'Hapus data siswa {{ addslashes($s->nama) }}?')">
            ...
            <form action="/admin/siswa/{{ $s->nisn }}/update" method="POST">
                ...
                <div class="modal-body p-4">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control bg-light border-0 fw-bold" value="{{ $s->nisn }}" disabled>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">NAMA LENGKAP</label>
                        <input type="text" name="nama" class="form-control bg-light border-0" value="{{ $s->nama }}" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label small fw-bold text-muted">KELAS</label>
                        <input type="text" name="kelas" class="form-control bg-light border-0" value="{{ $s->kelas }}" required>
                    </div>
                    <div class="mb-0">
                        <label class="form-label small fw-bold text-muted">KATA SANDI BARU</label>
                        <input type="password" name="password" class="form-control bg-light border-0" placeholder="Biarkan kosong jika tetap">
                        <small class="text-muted mt-2 d-block"><i class="bi bi-info-circle me-1"></i> Kosongkan jika tidak ingin mengganti password.</small>
                    </div>
                </div>
